...
class RouterList extends Component {
    ...
    makeCheked(route, stop) {
        if (document.getElementById(route + stop + "defaultCheck1").checked === true) {
            ...
            document.getElementById(route + stop).style.backgroundColor = "#11F46C";
        }
        else {
                ...
                <nav className="navbar bg-primary justify-content-between">
                    ...
                    <ul className="nav justify-content-end bg-primary">
                        <li className="nav-item nav-link mr-3">
                            <IconButton onClick={this.LogOutUser} name="Log Out" aria-label="logout" ClassName="pull-right btn btn-block btn-danger">
                                <i class="fas fa-sign-out-alt" style={{ fontSize: '43px' }} ></i>
                            </IconButton>
                        </li>
                    </ul>
                </nav>
